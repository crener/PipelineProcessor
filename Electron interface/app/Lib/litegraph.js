...
// *************************************************************
    ...
    var LiteGraph = global.LiteGraph = {
        ...
        registerRawNodeType: function (category, title, desc, inputs, outputs, useValue, defaultValue) {
            var nodeObject = {};
            nodeObject.title = title;
            nodeObject.input = inputs;
            nodeObject.output = outputs;
            nodeObject.desc = desc;
            nodeObject.useValue = useValue;
            nodeObject.value = useValue && defaultValue ? defaultValue : "";

            var pos = category.lastIndexOf("/");
            nodeObject.category = category.substr(0, pos);
            nodeObject.type = category;

            if (LiteGraph.debug)
                console.log("Node registered: " + node.category);

            this.registered_node_types[category] = nodeObject;
            this.Nodes[title] = nodeObject;
        },
        ...
        createNode: function (type, title, options) {
            ...
            if (!base_class.prototype) {
                return LiteGraph.createRawNode(type, options);
            }
        ...
        createRawNode: function (type, options) {
            var raw = this.registered_node_types[type];
            if (!raw) {
                if (LiteGraph.debug)
                    console.log("GraphNode type \"" + type + "\" not registered.");
                return null;
            ...
            //create and store empty object for Json based nodes
            var empty = this.registered_node_types["_Shell"];
            if (!empty) {
                var pro = function () {
                };
                ...
                for (var i in LGraphNode.prototype)
                    if (!pro.prototype[i])
                        pro.prototype[i] = LGraphNode.prototype[i];

                this.registered_node_types["_Shell"] = pro;
                empty = pro;
            }
            ...
            if (raw.output) {
                for (var i in raw.output) {
                    node.outputs.push({
                        name: raw.output[i].name,
                        type: raw.output[i].type,
                        links: null
                    })
                }
            }
            ...
            if (raw.input) {
                for (var i in raw.input) {
                    node.inputs.push({
                        name: raw.input[i].name,
                        type: raw.input[i].type,
                        links: null
                    })
                }
            }

            if (!node.title) node.title = raw.title;
            if (!node.properties) node.properties = {};
            if (!node.properties_info) node.properties_info = [];
            if (!node.flags) node.flags = {};
            if (!node.size) node.size = node.computeSize();
...
//This is more internal, it computes the order and returns it
     ...
     * @param {String} eventname the name of the event (generic to be called)
...
*/
    ...
    LGraphNode.prototype.serialize = function () {
        var o = {
            ...
            mode: this.mode,
            value: this.useValue ? this.value : ""
        };
...
//LGraphNode.prototype.unserialize = generic(info) {} //this cannot be done from within, must be done in LiteGraph
...
//safe LGraphNode action execution (not sure if safe)
    /*
LGraphNode.prototype.executeAction = generic(action)
{
	...
	if( typeof(this[func_name]) != "generic")
	{
...
//this is useful if you plant to render 3D objects inside your nodes
	...
	*/
    ...
    /*
LGraphCanvas.prototype.UIinit = generic()
{
	var that = this;
	$("#node-console input").change(generic(e)
	{
		if(e.target.value == "")
			return;
		...
		else if (e.target.value == "info")
		{
			...
			for(var i in node)
			{
				if(typeof(node[i]) == "generic" && LGraphNode.prototype[i] == null && i.substr(0,2) != "on" && i[0] != "_")
					node.trace(" + " + i);
			}
		}
		else
		{
			try
			{
				eval("var _foo = generic() { return ("+e.target.value+"); }");
...
*/
	...
	*/
    ...
    LGraphCanvas.prototype.processMouseDown = function (e) {
        ...
        {
            if (!e.shiftKey) //REFACTOR: integrate with generic
            {
                    ...
                    if (this.connecting_output.type == LiteGraph.EVENT && this.isOverNodeBox(node, e.canvasX, e.canvasY)) {
                        ...
                        if (slot != -1) {
                            ...
                            if (this.connecting_output.type == LiteGraph.EVENT)
                                this.connecting_node.connect(this.connecting_slot, node, LiteGraph.EVENT);
                            else if (input && !input.link && input.type == this.connecting_output.type) //toLowerCase missing
...
//called if the graph doesnt have a default drop item behaviour
    ...
    LGraphCanvas.prototype.drawFrontCanvas = function () {
        ...
        if (ctx.finish2D) //this is a generic I use in webgl renderer
            ctx.finish2D();
	...
	//*/
        ...
        if (!no_title) {
            ...
            if (shape == "round") {
                ...
                //draw + or - inside circle
                ctx.fillStyle = LiteGraph.NODE_TITLE_COLOR;
                ctx.globalAlpha = old_alpha;
                if (!node.flags.collapsed)
                    ctx.fillText("-", title_height * 0.65, title_height * -0.33);
                else ctx.fillText("+", title_height * 0.57, title_height * -0.28);
            }
            else {
                ...
                ctx.globalAlpha = old_alpha;
                if (!node.flags.collapsed)
                    ctx.fillText("-", title_height * 0.39, title_height * -0.32);
                else ctx.fillText("+", title_height * 0.32, title_height * -0.28);
            }
...
//OPTIMIZE THIS: precatch connections position instead of recomputing them every time
    ...
    /*
LGraphCanvas.prototype.resizeCanvas = generic(width,height)
{
...
*/
    ...
    LGraphCanvas.prototype.resize = function (width, height) {
        if (!width && !height) {
            var details = win.remote.BrowserWindow.getFocusedWindow();
            if (!details) return;

            var bounds = details.getContentBounds();
            width = this.canvas.parentElement.clientWidth;

            //Todo work out height dynamically
            var otherElementHeight = 80;
            height = bounds.height - (otherElementHeight + 20);
        }
    ...
    LGraphCanvas.onMenuAdd = function (node, e, prev_menu, canvas, first_event) {
        ...
        var menu = LiteGraph.createContextMenu(entries, {
            event: e,
    ...
    LGraphCanvas.onShowMenuNodeProperties = function (node, e, prev_menu) {
        ...
        for (var i in node.properties) {
            ...
            entries.push({
                content: "<span class='property_name'>" + i + "</span>" + "<span class='property_value'>" + value + "</span>",
                value: i
            });
        }
        if (!entries.length)
    ...
    LGraphCanvas.onMenuNodeMode = function (node, e, prev_menu) {
        LiteGraph.createContextMenu(["Always", "On Event", "Never"], {
            event: e,
            callback: inner_clicked,
            from: prev_menu
        });
    ...
    LGraphCanvas.prototype.getNodeMenuOptions = function (node) {
        ...
        //Note, null in the options array is a separator
        ...
        if (node.getMenuOptions)
            ...
            options = [
                ...
                //null,
                {content: "Rename", callback: LGraphCanvas.onShowTitleEditor},
...
//API *************************************************
//generic roundRect(ctx, x, y, width, height, radius, radius_low) {
...
//	format of a hex triplet - the kind we use for HTML colours. The generic
...
//Give a array with three values as the argument and the generic will return
//	the corresponding hex triplet.
    ...
    /*
LiteGraph.createNodetypeWrapper = generic( class_object )
{
